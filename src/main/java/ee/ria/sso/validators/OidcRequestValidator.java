...
public class OidcRequestValidator {
    ...
    private static void validateParameter(HttpServletRequest request, OidcRequestParameter parameter) {
    ...
    private String validateRedirectUriIsRegistered(HttpServletRequest request) {
        ...
        validateParameter(request, OidcRequestParameter.CLIENT_ID);

        String clientId = request.getParameter(OidcRequestParameter.CLIENT_ID.getParameterKey());
        OAuthRegisteredService registeredService = OAuth20Utils.getRegisteredOAuthService(servicesManager, clientId);
        if (registeredService != null && registeredService.getAccessStrategy().isServiceAccessAllowed()) {
            String urlPattern = registeredService.getServiceId();
            String redirectUrl = request.getParameter(OidcRequestParameter.REDIRECT_URI.getParameterKey());
            if (redirectUrl != null && redirectUrl.matches(urlPattern)) {
                return redirectUrl;
            } else {
                throw new InvalidRequestException(OidcRequestParameter.REDIRECT_URI, "invalid_request",
    ...
    private static void assertParameterHasSingleValue(final HttpServletRequest request, final OidcRequestParameter parameter) {
    ...
    private static void assertParameterValueNotEmpty(final HttpServletRequest request, final OidcRequestParameter parameter) {
    ...
    private static void validateScopeValue(final HttpServletRequest request) {
        String scope = request.getParameter(OidcRequestParameter.SCOPE.getParameterKey());
        ...
        if (scopes.isEmpty() || !scopes.contains(TaraScope.OPENID.getFormalName())) {
            throw new InvalidRequestException(OidcRequestParameter.SCOPE, "invalid_scope", String.format(
    ...
    private static void validateResponseType(final HttpServletRequest request) {
        String responseType = request.getParameter(OidcRequestParameter.RESPONSE_TYPE.getParameterKey());
        if (!"code".equals(responseType)) {
            throw new InvalidRequestException(OidcRequestParameter.RESPONSE_TYPE, "unsupported_response_type", String.format(
    ...
    private static void validateAcrValues(final HttpServletRequest request) {
        String acrValues = request.getParameter(OidcRequestParameter.ACR_VALUES.getParameterKey());
        ...
        if (acrValues != null) {
            ...
            if (!allowedValues.contains(acrValues)) {
                throw new InvalidRequestException(OidcRequestParameter.ACR_VALUES, "unsupported_acr_values", String.format(
    ...
    public static class InvalidRequestException extends RuntimeException {
        ...
        private final OidcRequestParameter parameter;
        private final String errorCode;
        private final String errorDescription;

        public InvalidRequestException(OidcRequestParameter parameter, String errorCode, String errorDescription) {
        ...
        public OidcRequestParameter getInvalidParameter() {
            return parameter;
        }
    }
}
