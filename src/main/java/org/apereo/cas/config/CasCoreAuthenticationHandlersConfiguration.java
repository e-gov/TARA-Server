...
public class CasCoreAuthenticationHandlersConfiguration {
	...
	public AuthenticationHandler acceptUsersAuthenticationHandler() {
		final AcceptAuthenticationProperties acceptAuthenticationProperties =
				casProperties.getAuthn().getAccept();
		final AcceptUsersAuthenticationHandler h =
				new AcceptUsersAuthenticationHandler(acceptAuthenticationProperties.getName(),
						servicesManager,
						acceptUsersPrincipalFactory(), null);
		return h;
	}
	...
	private Map<String, String> getParsedUsers() {
		...
		if (StringUtils.isNotBlank(usersProperty) && usersProperty.contains(pattern.pattern())) {
			return Stream.of(usersProperty.split(","))
					.map(pattern::split)
					.collect(Collectors.toMap(userAndPassword -> userAndPassword[0],
							userAndPassword -> userAndPassword[1]));
		}
	...
	@EnableConfigurationProperties(CasConfigurationProperties.class)
	public class ProxyAuthenticationEventExecutionPlanConfiguration
			implements AuthenticationEventExecutionPlanConfigurer {
		@Override
		public void configureAuthenticationExecutionPlan(
				final AuthenticationEventExecutionPlan plan) {
			plan.registerAuthenticationHandlerWithPrincipalResolver(proxyAuthenticationHandler(),
	...
	public class JaasAuthenticationEventExecutionPlanConfiguration
		...
		public void configureAuthenticationExecutionPlan(
				final AuthenticationEventExecutionPlan plan) {
			plan.registerAuthenticationHandlerWithPrincipalResolvers(jaasAuthenticationHandlers(),
