...
| **Field**       | **Description** |
| :---------------- | :---------- |
| **request** | Request method and URL in the form of `METHOD protocol://server_name:server_port/request_uri`. For example `POST https://koogelmoogel.net:443/login`. |
...
| `ID_TOKEN_REQUEST_HANDLING_SUCCESS` | Request was successfully processed and an ID-token was sent as a response. |
| `ID_TOKEN_REQUEST_HANDLING_FAILED` | An error occurred while processing the request or sending the response. |
